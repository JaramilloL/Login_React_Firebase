...
const Login = () => {
  ...
  const { login, loginAcces, loginGoogle, resetPassword } =
  ...
  const handleResetPassword = async () => {
    ...
    if (!userEmail) {
		alert("You have not added an email");
  ...
  return (
    <div className="">
      <form
        onSubmit={onSubmit}
        className="w-25 m-auto mt-5 dark-glass-container"
      >
        ...
        <div className="text-center mt-5 justify-content-betteewn">
          ...
          />
          <a href="#" className="h6 text-dark" onClick={handleResetPassword}>
            Forgot password?
          </a>
          <p className="text-decoration-underline text-dark"> or </p>
          <Link to={"/register"} className="h6 text-dark">
            Register
          </Link>
        </div>
      </form>
      ...
      <div className="text-center mt-2">
        <button
          ...
          className="btn border w-25 m-auto bg-google"
        >
